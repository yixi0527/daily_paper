...
class CellPressAdapter(PublisherAdapter):
    def build_sources_for_category(self, journal: Journal, category: str) -> list[SourceSpec]:
        ...
        if category == "current_issue" and journal.rss_current_issue_url:
            sources.append(
                SourceSpec(name="cellpress_rss", kind="rss", url=journal.rss_current_issue_url)
            )
        if category == "online_first" and journal.rss_online_first_url:
            sources.append(
                SourceSpec(name="cellpress_rss", kind="rss", url=journal.rss_online_first_url)
            )
